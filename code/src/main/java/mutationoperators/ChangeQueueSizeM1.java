...
public class ChangeQueueSizeM1 extends MutationOperator {

    public ChangeQueueSizeM1(MutationContext context) {
        super(context);
    }
    ...
    protected Document mutate(QueueNetwork qn) {
        ...
        for (Server queue : qn.getNodes().stream().filter(n -> n instanceof Server).map(n -> (Server) n).collect(Collectors.toList())) {
            ...
            if (oldSize == 1 || oldSize == -1) {
                continue; // 1-1 = 0 impossible and infinite already mutated
            }
