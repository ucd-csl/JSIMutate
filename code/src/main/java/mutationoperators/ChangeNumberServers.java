...
public class ChangeNumberServers extends MutationOperator {

    public ChangeNumberServers(MutationContext context) {
        super(context);
    }
    ...
    protected Document mutate(QueueNetwork qn) {
        ...
        for (Server server : qn.getNodes().stream().filter(n -> n instanceof Server).map(n -> (Server) n).collect(Collectors.toList())) {
            ...
            if (oldNumberServers == 1) {
                continue; // 1/2 = 1 so equivalent mutant
            }
            int newNumberServers = oldNumberServers / 2;
